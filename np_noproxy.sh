...
check_port() {
  ...
  if ! grep -q 'no_check_used' <<< "$NO_CHECK_USED"; then
    if [ $(type -p nc) ]; then
      ...
      netstat -nltup 2>/dev/null | grep -q ":$PORT " && return 1
    elif [ $(type -p ss) ]; then
...
select_language() {
  ...
  if [ -n "$ARGS_LANGUAGE" ]; then
    case "$ARGS_LANGUAGE" in
      1|zh|CN|cn|chinese|C|c) L=C ;;
      2|en|EN|english|E|e) L=E ;;
      *) L=C ;;
    esac
...
# =====================================================================
# FIX 2: get_latest_version 在无法获取远程版本时使用硬编码回退版本
# =====================================================================
get_latest_version() {
  ...
  # 回退到硬编码版本（当无法从远程获取时）
  [ -z "$STABLE_LATEST_VERSION" ] && STABLE_LATEST_VERSION="v1.15.0"
  [ -z "$DEV_LATEST_VERSION" ]    && DEV_LATEST_VERSION="v1.15.0"
  [ -z "$LTS_LATEST_VERSION" ]    && LTS_LATEST_VERSION="v1.15.0"
...
on_off() {
  if [ "$IN_CONTAINER" = 1 ] || [ "$SERVICE_MANAGE" = "none" ]; then
    if [ $(type -p pgrep) ]; then
      if pgrep -laf "nodepass" | grep -vE "<defunct>|grep" | grep -q "nodepass"; then RUNNING=1; else RUNNING=0; fi
    else
      if ps -ef | grep -vE "grep|<defunct>" | grep -q "nodepass"; then RUNNING=1; else RUNNING=0; fi
    fi
  elif [ "$SERVICE_MANAGE" = "systemctl" ]; then
    systemctl is-active nodepass >/dev/null 2>&1 && RUNNING=1 || RUNNING=0
  elif [ "$SERVICE_MANAGE" = "rc-service" ]; then
    rc-service nodepass status | grep -q "started" && RUNNING=1 || RUNNING=0
  elif [ "$SERVICE_MANAGE" = "init.d" ]; then
    [ -f "/var/run/nodepass.pid" ] && kill -0 $(cat "/var/run/nodepass.pid" 2>/dev/null) >/dev/null 2>&1 && RUNNING=1 || RUNNING=0
  fi

  if [ "$RUNNING" = 1 ]; then
    stop_nodepass && info " $(text 42) "
  else
    start_nodepass && info " $(text 43) "
  fi
}
...
upgrade_nodepass() {
  ...
  [ "$VERSION_TYPE_TEXT" = "$(text 67)" ] && [ "$HAS_STABLE_UPGRADE" = 1 ] && NEED_RESTART=1
  [ "$VERSION_TYPE_TEXT" = "$(text 66)" ] && [ "$HAS_DEV_UPGRADE" = 1 ] && NEED_RESTART=1
  [ "$VERSION_TYPE_TEXT" = "$(text 98)" ] && [ "$HAS_LTS_UPGRADE" = 1 ] && NEED_RESTART=1
  [ "$HAS_STABLE_UPGRADE" = 1 ] && [ "$HAS_DEV_UPGRADE" = 1 ] && [ "$HAS_LTS_UPGRADE" = 1 ] && NEED_RESTART=1
  ...
  if [ "$NEED_RESTART" = 1 ]; then
    ...
    if start_nodepass; then
      ...
      if start_nodepass; then info " $(text 54) "; else error " $(text 55) "; fi
    fi
  else
    info " $(text 52) "
    rm -f "$WORK_DIR/np-stb.old" "$WORK_DIR/np-dev.old" "$WORK_DIR/np-lts.old"
...
switch_nodepass_version() {
  ...
  case "$SWITCH_CHOICE" in
    1) TARGET_INDEX=${AVAILABLE_INDICES[0]} ;;
    2) TARGET_INDEX=${AVAILABLE_INDICES[1]} ;;
    *) info " $(text 103)"; return 0 ;;
  esac
...
parse_args() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --server_ip) ARGS_SERVER_IP="$2"; shift 2 ;;
      --user_port) ARGS_PORT="$2"; shift 2 ;;
      --prefix) ARGS_PREFIX="$2"; shift 2 ;;
      --tls_mode) ARGS_TLS_MODE="$2"; shift 2 ;;
      --language) ARGS_LANGUAGE="$2"; shift 2 ;;
      --version) ARGS_VERSION="$2"; shift 2 ;;
      --cert_file) ARGS_CERT_FILE="$2"; shift 2 ;;
      --key_file) ARGS_KEY_FILE="$2"; shift 2 ;;
      *) shift ;;
    esac
  done
}
...
install() {
  ...
  # =====================================================================
  # FIX 4: 每个版本使用独立临时目录，避免并发下载时文件相互覆盖
  # =====================================================================
  ...
  while true; do
    ...
    if [ -z "$PORT" ]; then
      PORT=$(get_random_port); info " $(text 37) $PORT"; break
    else
      check_port "$PORT" "check_used"
      local PORT_STATUS=$?
      if [ "$PORT_STATUS" = 2 ]; then unset ARGS_PORT PORT; warning " $(text 41) "
      elif [ "$PORT_STATUS" = 1 ]; then unset ARGS_PORT PORT; warning " $(text 36) "
      else break
      fi
    fi
  done
  ...
  if [[ "$REMOTE_SERVER_INPUT" =~ ^([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}$ ]]; then
    CMD_SERVER_IP="127.0.0.1"; URL_SERVER_IP="[$REMOTE_SERVER_INPUT]"
    ...
    SERVER_IP="$SERVER_INPUT"; URL_SERVER_IP="[$SERVER_IP]"; URL_SERVER_PORT="$PORT"
  ...
  while true; do
    ...
    if grep -q '^[a-z0-9/]*$' <<< "$PREFIX"; then
      ...
      unset ARGS_PREFIX PREFIX; warning " $(text 61) "
    fi
  done
  [ -z "$PREFIX" ] && PREFIX="api"
  ...
  if [ -n "$ARGS_TLS_MODE" ]; then
    ...
    [[ ! "$TLS_MODE" =~ ^[0-2]$ ]] && TLS_MODE=0
  else
    hint "\n (5/5) $(text 15) "; hint " $(text 16) "
    reading "\n $(text 38) " TLS_MODE
    if [ -z "$TLS_MODE" ]; then TLS_MODE=0
    elif [[ ! "$TLS_MODE" =~ ^[0-2]$ ]]; then warning " $(text 17) "; exit 1
    fi
  fi

  if [ "$TLS_MODE" = "2" ]; then
    if [ -n "$ARGS_CERT_FILE" ]; then
      [ ! -f "$ARGS_CERT_FILE" ] && error " $(text 25) $ARGS_CERT_FILE"
      CERT_FILE="$ARGS_CERT_FILE"
    else
      while true; do
        reading " $(text 23) " CERT_FILE
        [ -f "$CERT_FILE" ] && break || warning " $(text 25) $CERT_FILE"
      done
    fi
    if [ -n "$ARGS_KEY_FILE" ]; then
      [ ! -f "$ARGS_KEY_FILE" ] && error " $(text 26) $ARGS_KEY_FILE"
      KEY_FILE="$ARGS_KEY_FILE"
    else
      while true; do
        reading " $(text 24) " KEY_FILE
        [ -f "$KEY_FILE" ] && break || warning " $(text 26) $KEY_FILE"
      done
    fi
  ...
  # =====================================================================
  # FIX 5: 检查独立目录中的文件，给出准确的错误提示
  # =====================================================================
  if [[ -s "$TEMP_DIR/np-stb" && -s "$TEMP_DIR/np-dev" && -s "$TEMP_DIR/np-lts" && -s "$TEMP_DIR/qrencode" ]]; then
    info " $(text 19) "
  elif [[ ! -s "$TEMP_DIR/np-stb" && ! -s "$TEMP_DIR/qrencode" ]]; then
    local APP="NodePass, QRencode" && error "\n $(text 9) "
  elif [ ! -s "$TEMP_DIR/np-stb" ]; then
    local APP="NodePass" && error "\n $(text 9) "
  elif [ ! -s "$TEMP_DIR/qrencode" ]; then
    local APP="QRencode" && warning "\n $(text 9) "
    # qrencode 非必须，继续安装
  fi
  ...
  # =====================================================================
  # FIX 6: 从各自独立目录移动文件，不再有覆盖问题
  # =====================================================================
  mv "$TEMP_DIR/np-stb" "$WORK_DIR/np-stb"
  mv "$TEMP_DIR/np-dev" "$WORK_DIR/np-dev"
  mv "$TEMP_DIR/np-lts" "$WORK_DIR/np-lts"
  [ -s "$TEMP_DIR/qrencode" ] && mv "$TEMP_DIR/qrencode" "$WORK_DIR/" || touch "$WORK_DIR/qrencode"
  chmod +x $WORK_DIR/np-stb $WORK_DIR/np-dev $WORK_DIR/np-lts $WORK_DIR/qrencode 2>/dev/null
    ...
    if grep -q '.' <<< "$REMOTE_SERVER_INPUT" && grep -q '.' <<< "$REMOTE_PORT_INPUT"; then
      if [ "$DOWNLOAD_TOOL" = "curl" ]; then
        local CREATE_NEW_INSTANCE_ID=$(curl -ksS -X 'POST' \
          ...
          -d "{\"url\": \"client://${REMOTE_PASSWORD_INPUT}${URL_SERVER_IP}:${TUNNEL_PORT_INPUT}/127.0.0.1:${PORT}\"}" 2>&1 | sed 's/{"id":"\([0-9a-f]\{8\}\)".*/\1/')
        grep -q "^[0-9a-f]\{8\}$" <<< "${CREATE_NEW_INSTANCE_ID}" && curl -X 'PATCH' "http://127.0.0.1:${PORT}/${PREFIX}/v1/instances/${CREATE_NEW_INSTANCE_ID}" -H "X-API-KEY: ${KEY}" -d '{ "restart": true }' >/dev/null 2>&1
      else
        ...
        grep -q "^[0-9a-f]\{8\}$" <<< "${CREATE_NEW_INSTANCE_ID}" && wget --no-check-certificate --method=PATCH --header="X-API-KEY: ${KEY}" --body-data='{ "restart": true }' "http://127.0.0.1:${PORT}/${PREFIX}/v1/instances/${CREATE_NEW_INSTANCE_ID}" >/dev/null 2>&1
      fi
...
stop_service() {
  ...
  { kill \$(cat \$PID 2>/dev/null); rm -f \$PID; } >/dev/null 2>&1
}
start() { start_service; }
stop() { stop_service; }
...
create_shortcut() {
  ...
  if [ "$DOWNLOAD_TOOL" = "curl" ]; then DOWNLOAD_COMMAND="curl -ksSL"
  else DOWNLOAD_COMMAND="wget --no-check-certificate -qO-"
  fi
...
uninstall() {
  if [ "$IN_CONTAINER" = 1 ] || [ "$SERVICE_MANAGE" = "none" ]; then
    if [ $(type -p pgrep) ]; then pgrep -f "nodepass" | xargs -r kill -9 >/dev/null 2>&1
    else ps -ef | grep -v grep | grep "nodepass" | awk '{print $2}' | xargs -r kill -9 >/dev/null 2>&1; fi
  elif [ "$SERVICE_MANAGE" = "systemctl" ]; then
    systemctl stop nodepass; systemctl disable nodepass
    rm -f /etc/systemd/system/nodepass.service; systemctl daemon-reload
  elif [ "$SERVICE_MANAGE" = "rc-service" ]; then
    rc-service nodepass stop; rc-update del nodepass; rm -f /etc/init.d/nodepass
  elif [ "$SERVICE_MANAGE" = "init.d" ]; then
    /etc/init.d/nodepass stop; /etc/init.d/nodepass disable; rm -f /etc/init.d/nodepass
  fi
...
change_intranet_penetration_server() {
  ...
  if [ "$DOWNLOAD_TOOL" = "curl" ]; then
    curl -ksS -X 'PUT' \
      "${HTTP_S}://127.0.0.1:${PORT}/${PREFIX}/v1/instances/${INSTANCE_ID}" \
      -H 'accept: application/json' -H "X-API-Key: ${KEY}" -H 'Content-Type: application/json' \
      -d "{\"url\": \"client://${REMOTE_PASSWORD_INPUT}${REMOTE_SERVER_INPUT}:${TUNNEL_PORT_INPUT}/127.0.0.1:${PORT}\"}" &>/dev/null
  else
    wget --no-check-certificate -qO- --method=PUT \
      --header="accept: application/json" --header="X-API-Key: ${KEY}" --header="Content-Type: application/json" \
...
change_api_key() {
  ...
  if [ "$DOWNLOAD_TOOL" = "curl" ]; then
    local RESPONSE=$(curl -ks -X 'PATCH' "${HTTP_S}://127.0.0.1:${PORT}/${PREFIX}/v1/instances/********" \
      -H "accept: application/json" -H "X-API-Key: ${KEY}" -H "Content-Type: application/json" \
      -d '{"action": "restart"}')
  else
  ...
  if [ "${#NEW_KEY}" = 32 ]; then
    info " $(text 63) "; get_api_url output; info " $(text 40) $NEW_KEY"
    [ "$NEED_STOP" = 1 ] && stop_nodepass; return 0
  else
...
menu_setting() {
  ...
  if [ "$INSTALL_STATUS" = 2 ]; then
    ...
    get_api_key; get_api_url; get_uri; get_local_version all
...
main() {
  ...
  case "$1" in
    -i)
      ...
      ;;
    -u) [ "$INSTALL_STATUS" = 2 ] && warning " ${E[59]}\n ${C[59]} " || uninstall ;;
    -v) [ "$INSTALL_STATUS" = 2 ] && warning " ${E[59]}\n ${C[59]} " || upgrade_nodepass ;;
    -t) [ "$INSTALL_STATUS" = 2 ] && warning " ${E[59]}\n ${C[59]} " || switch_nodepass_version ;;
    -o) [ "$INSTALL_STATUS" = 2 ] && warning " ${E[59]}\n ${C[59]} " || on_off $INSTALL_STATUS ;;
    -s)
      if [ "$INSTALL_STATUS" = 2 ]; then
        warning " ${E[59]}\n ${C[59]} "
      else
      ...
      ;;
    -k) [ "$INSTALL_STATUS" = 2 ] && warning " ${E[59]}\n ${C[59]} " || change_api_key $INSTALL_STATUS ;;
    -c)
      if [ "$INSTALL_STATUS" != 2 ]; then
        get_api_url; get_api_key; change_intranet_penetration_server
      else
        warning " ${E[59]}\n ${C[59]} "
      fi
      ;;
